...
if [ -e ${TMP_DIR} ]; then
  if [ ! -d ${TMP_DIR} ]; then
    echo -e "\n  Temp directory ${TMP_DIR} exists but is not a directory. STOP"
    exit 1
  fi
else
  echo -e "\n  Temp directory ${TMP_DIR} does not exist. Creating"
  mkdir ${TMP_DIR}
fi

if [ -e ${OUT_DIR} ]; then
  if [ ! -d ${OUT_DIR} ]; then
    echo -e "\n  Output directory ${OUT_DIR} exists but is not a directory. STOP"
    exit 1
  fi
else
...
cdo -s chname,LUFRAC_16,OPEN -mul -selname,LUFRAC_16 ${TMP_DIR}/cn06.nc -mulc,-1 -subc,1 -selname,LWMASK ${TMP_DIR}/cn06.nc ${TMP_DIR}/tmp01.nc

## Remove some water grind cells from the new OPEN variable
# Otherwise, large lakes and river deltas who we open ocean regions.
case ${GRID_NAME} in
    "CN06")
       cdo -s setcindexbox,0,1,90,55,100 -setcindexbox,0,1,68,0,100 -setcindexbox,0,90,100,1,10 -setcindexbox,0,110,115,10,24 -setcindexbox,0,125,140,84,100 ${TMP_DIR}/tmp01.nc ${TMP_DIR}/tmp02.nc
       ;;
    *)
...
cdo -s -O merge -chname,OPEN,SURF -mulc,0 -selname,OPEN ${TMP_DIR}/tmp02.nc -selname,OPEN ${TMP_DIR}/tmp02.nc -chname,OPEN,MASK -ifthenelse -eqc,0 -selname,OPEN ${TMP_DIR}/tmp02.nc -mulc,0 -selname,OPEN ${TMP_DIR}/tmp02.nc -ifthenelse -eqc,1 -selname,OPEN ${TMP_DIR}/tmp02.nc -addc,2 -mulc,0 -selname,OPEN ${TMP_DIR}/tmp02.nc -addc,1 -mulc,0 -selname,OPEN ${TMP_DIR}/tmp02.nc ${TMP_DIR}/tmp03.nc
